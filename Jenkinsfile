node ("openshift-test-pipeline-slave") {
    ...
    try {
        ...
        stage("OpenShift load testing") {
            sh("""
                ssh -o StrictHostKeyChecking=no -i ssh_key cloud-user@\$BASTIONIP \
                    "ansible-playbook -i /usr/share/ansible/openshift-deployment-ansible/openshift-ansible-hosts /home/cloud-user/openshift-tooling/locust/locust_setup.yaml -e "@/home/cloud-user/openshift-tooling/locust/vars.yml" -e domainSuffix=\"\$DOMAINSUFFIX\""
            """)
        }
        ...
        */
        ...
        slackSend color: "#7ea8c8", message: "openshift-test-pipeline finished: Job name: ${env.JOB_NAME} Build number: ${env.BUILD_NUMBER} - <${env.BUILD_URL}|Jenkins build URL>"
